...
retext = new Retext();
retextWithMetaphone = new Retext().use(metaphone);
...
suite('retext w/o retext-metaphone', function () {
...
suite('retext w/ retext-metaphone', function () {
    bench('A paragraph (5 sentences, 100 words)', function (done) {
        retextWithMetaphone.parse(paragraph, done);
    });

    bench('A section (10 paragraphs, 50 sentences, 1,000 words)',
        function (done) {
            retextWithMetaphone.parse(section, done);
        }
    );
});
